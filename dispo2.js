...
<div id="tm-tabs">
  ...
  <button class="tm-tab" data-tab="auto">Automation</button>
</div>
<div id="tm-tab-contents">
    ...
    const autoCont = panel.querySelector('#tm-auto-container');
    const autoBtn = document.createElement('button');
    autoBtn.textContent = "Run Endeavor Auto";
    autoBtn.style.width = "100%";
    autoBtn.style.margin = "5px";

    autoBtn.onclick = () => {
        if (window.runEndeavorAuto) {
            window.runEndeavorAuto();
        } else {
            console.log("Automation script not loaded");
        }
    };

    autoCont.appendChild(autoBtn);
    ...
    // === CONTROL # AUTO COPY (FIXED) ===

    const attachControlCopy = () => {
        // specifically look for text containing "Control #:"
        const candidates = Array.from(document.querySelectorAll('*'))
        .filter(el => el.children.length === 0); // only leaf nodes (prevents big containers)
        ...
        candidates.forEach(el => {
            ...
            // style ONLY the text itself
            el.style.cursor = "pointer";
            el.style.display = "inline-block"; // prevents weird overlay behavior

            // remove ugly tooltip behavior
            el.removeAttribute("title");

            el.addEventListener('click', (e) => {
                e.stopPropagation(); // prevents parent weird clicks

                GM_setClipboard(controlValue);

                // clean feedback (no layout shift)
